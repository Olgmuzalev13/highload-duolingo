...
| Главные функции приложения    | graceful degradation, feature toggle, fallback logic       | graceful degradation - отключение второстепенных функций при ошибках<br>feature toggle - включение/отключение функционала без перезапуска<br>fallback logic - резервные сценарии поведения |
...
1. ML Individual Learning Curves - отдельный микросервис выполняющий трудоемкую работу на основе ML. ML модели часто меняются обновляются и откатываются, а небольшая задержка в повышении уровня языкового знания пользователя будет не очень критична, если со временем это исправится, исправлять планируется с помощью промежуточной базы данных, которая перезаправшивает все запросы при восстановлении работы ML Individual Learning Curves. Микросервис Lesson Collector при падении бедт брать данные об уровне пользователя сохранненые в БД.
2. Mistakes - самостоятельный микросевис активно участвующий в каждом формировании урока, однако может отказать - но если в некоторых уроках не будет отработки ошибок пользователя это заденет гораздо меньше, чем если у него перестанут грузиться все уроки.
3. Notification Bandit - микросервис выполняющий сложеный ML анализ, однако если он откажет лучше обратиться к проверенным текстам уведомлений хранимых в отдельной БД чем не присылать уведомления вовсе.
4. API Gateway - является единой точкой отказа, поэтому разработан ALT Gateway который объединяет в себе основные функции сервиса в упрощенном виде, самостоятельно обращаясь к микросервисам на случай если они работают и сохраняющий необходимые метаданные, для восстановления прогресса пользоватлей после старта полноценного пути.
